...
{
    dxdt[0] = -p_beta/N_pop*x[0]*x[2];
    dxdt[1] = p_beta/N_pop*x[0]*x[2] - p_gamma*x[1];
    dxdt[2] = p_gamma*x[1]-p_alpha*x[2];
    dxdt[3] = p_alpha*x[2];
}
...
int main(int /* argc */ , char** /* argv */ )
{
    ...
    std::ofstream yFile;
    yFile.open("/home/deb/Documents/MCMC_git/Custom_MCMC/Data/SEIR_I0_1000.csv");

    yFile << N_observations << '\n' << DT << '\n';
    for (size_t i=0; i<=N_observations; i++)
    {
        yFile << x_vec[i][0] << ',' << x_vec[i][1] << ',' << x_vec[i][2] << ',' << x_vec[i][3] << '\n';
    }
