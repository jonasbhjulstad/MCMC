...
class DifferentialEquation(Op):
    ...
    def _system(self, Y, t, p):
        ...
        """
        dydt = self._augmented_func(Y[: self.n_states], t, p)
        return dydt
    ...
    def __call__(self, y0, theta, return_sens=False, **kwargs):
        ...
        states = super().__call__(y0, theta, **kwargs)

        if theano.config.compute_test_value != "off":
            # compute test values from input test values
            test_states = self._simulate(
    ...
    def perform(self, node, inputs_storage, output_storage):
        ...
        #output_storage[1][0] missing!
        output_storage[0][0] = self._simulate(y0, theta)
