...
namespace smc
{
    ...
    private:
        ...
        rng *pRng;
        //Separate generators for threads:
        // rng** threadRng;
        ...
        long T;
        ///Current iteration of smc
        long T_MCMC;
        ///Total number of time iterations per smc-step
        long T_max;
        ...
        double* propParam;
        ///Average particle weights for all time iterations in all smc iterations [][]
        double** pSumLogWeights;
        ///Total weight-factor for the smc particle run:
    ...
    public:
        ...
        ///Metropolis-Hastings smc-correction:
    ...
    sampler<Space>::sampler(long lSize, long T_maximum,long N_MCMC_iterations, long N_param, double* param_init, long N_param_ODE)
    {
        ...
        // for (int i; i < N; i++)
        // {
        //     threadRng[i] = new rng(gsl_rng_taus);
        // }
        N = lSize;
    ...
    void sampler<Space>::IterateSingle(void)
    {
        ...
        for (int i = 0; i < N; i++){
            // std::cout << pParticles[i]->GetLogWeight() << std::endl;
    ...
    void sampler<Space>::MoveParticles(void)
    {
        for (int i = 0; i < N; i++)
        {
            // Adapter< auto, long, particle<Space>&, double*, rng*> tFun(pfMove, T, pParticles[i], propParam, threadRng[i]);
            // pfThreads[i] = new boost::thread(tFun);
    ...
    int sampler<Space>::Metropolis_Hastings(void)
    {
        ...
        int accept;
        // std::cout << "Alpha: " << alpha_Metropolis << std::endl;
        //Accept the proposal
