...
if __name__ == '__main__':
    ...
    weights = []
    for nu in nu_list:
        params.append(pd.read_csv("../Data/" + model + "/param_" + str(nu) + "_" + str(ll) + "_" + str(N_MCMC) + ".csv", names=['alpha', 'beta', 'gamma'], header=None))
    ...
    for param, nu, color in zip(reversed(params), reversed(nu_list), colors):
        ...
        x2, bin2, p2 = ax[2].hist(param['gamma'], bins=100, density=True, color=color)
        for item0, item1, item2 in zip(p0, p1, p2):
            ...
            ymax = max(ymax, x0_norm, x1_norm, x2_norm)


    ax[0].legend()
    _ = [x.set_ylim([0, ymax]) for x in ax]
    _ = [x.set_title(t) for t, x in zip(['alpha', 'beta', 'gamma'], ax)]
